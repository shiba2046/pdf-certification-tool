...
def add_text_box_to_pdf(
    input_pdf_path, output_pdf_path, vertical, horizontal, color, text
):
...
def process_pdf(
    ...
    if input_path is None:
        input_path = os.path.join(
            "uploads",
            input_file.filename if hasattr(input_file, "filename") else input_file,
        )
    if output_path is None:
        output_path = os.path.join(
            "processed", f"processed_{os.path.basename(input_path)}"
        )
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            output_file = process_pdf(
                file, vertical=vertical, horizontal=horizontal, color=color, text=text
            )
            return send_file(
                output_file, mimetype="application/pdf", as_attachment=False
            )
